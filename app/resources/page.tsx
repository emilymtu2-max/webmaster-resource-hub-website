...
export default function ResourcesPage() {
  ...
  const likedSet = useMemo(() => new Set(likedResources), [likedResources]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (stored) {
        setLikedResources(JSON.parse(stored) as string[]);
      }
    } catch {
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify(likedResources));
  }, [likedResources]);

  const toggleLike = (title: string) => {
    setLikedResources((current) => {
  ...
  const filteredResources = useMemo(() => {
    return resources.filter((resource) => {
      ...
      const matchesSearch =
        ...
        resource.about.toLowerCase().includes(search.toLowerCase());
  ...
  const resourcesToRender = useMemo(() => {
    ...
    if (showLikedOnly) {
      items = items.filter((resource) => likedSet.has(resource.title));
    }
    if (sortByLiked) {
      items = [...items].sort((a, b) => {
        const aPriority = likedSet.has(a.title) ? 0 : 1;
        const bPriority = likedSet.has(b.title) ? 0 : 1;
        return aPriority - bPriority;
      });
    }
    return items;
  }, [filteredResources, likedSet, showLikedOnly, sortByLiked]);
    ...
    <div className="min-h-screen flex flex-col">
      <section className="resources-hero p-6 bg-gray-50 text-center">
        ...
        <p className="max-w-2xl mx-auto text-gray-600">
          A curated collection of organizations, tools, and programs that support immigrant communities and promote education,
          legal access, and cultural awareness.
        </p>
      </section>
      ...
      <div className="resource-filters flex flex-wrap justify-center gap-2 p-4">
        {categories.map((cat) => (
          <button
            key={cat}
